...
```text
docker build -f DockerF1 -t devh_fullenv:u22 .

docker run -it --name devhname --net=host \
-e DISPLAY=$DISPLAY \
...
devh_fullenv:u22 bash

#-v $HOME:/home/$USER \

xhost +local:docker

docker start -ai devhname

docker exec -it devhname bash
